...
const { createTradeRequest } = require('../core/governance_integration');
...
async function testSimulation() {
  ...
  try {
    ...
    // Using smaller sizes to pass risk checks (max 30% position size, max $2000 asset exposure)
    const testTrades = [
      {
        strategy: 'momentum',
        pair: 'BTC/USD',
        action: 'buy',
        amount: 0.05, // ~$2250 (22.5% of $10k capital, under 30% limit)
        price: 45000,
        estimatedValue: 2250
      },
      {
        ...
        pair: 'ETH/USD',
        action: 'buy',
        amount: 0.5, // ~$1500 (15% of capital, under 30% limit)
        price: 3000,
        estimatedValue: 1500
      },
      {
    ...
    if (governance.eventLog) {
      ...
      if (tradeEvents.length > 0) {
        ...
        tradeEvents.slice(-3).forEach(event => {
          ...
          if (tradeEvent.metadata && tradeEvent.metadata.fees) {
